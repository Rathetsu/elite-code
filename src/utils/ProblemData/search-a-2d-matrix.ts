...
export const TestingFunctionSearch2DMatrix = (fun: any) => {
  try {
    const tests = [
      {
        matrix: [
    ...
    for (let i = 0; i < tests.length; i++) {
      const result = fun(tests[i].matrix, tests[i].target);
      assert.deepEqual(result, answers[i]);
    }
    return true;
  } catch (error: any) {
    console.log("Search-a-2d-matrix testing function error", error);
    throw new Error(error);
  }
};
const startingCodeSearch2DMatrixJS = `// Do not edit function name
...
export const search2DMatrix: Problem = {
  ...
  `,
  testingFunction: TestingFunctionSearch2DMatrix,
  startingCode: startingCodeSearch2DMatrixJS,
  startingFunctionName: "function searchMatrix",
};
